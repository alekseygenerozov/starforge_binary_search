...
for ii,row in enumerate(bin_ids):
    ...
    for jj in range(len(p1)):
        if ~np.isinf(p1[jj,0]) and ~np.isinf(p2[jj, 0]):
            closest_tag = close[ii][jj]
            tmp_close = path_lookup[closest_tag][jj]

            pbin = np.vstack((p1[jj], p2[jj]))
            pclose = np.vstack((p1[jj], p2[jj], tmp_close))
...
np.savez("{0}/tide_stars_close.npz".format(sys.argv[1]), tides_norm_series)
