...
with open(base + aa + "closest.p", "rb") as ff:
    closest = pickle.load(ff)
...
with open(base + aa + "/tide_stars.p", "rb") as ff:
    tides_stars_all = pickle.load(ff)

for ii,row in enumerate(bin_ids):
    bin_key = str(np.sort(list(bin_ids[ii])))
    row_tides_all = tides_stars_all[bin_key]
    tide_max_idx = np.where(row_tides_all == np.max(row_tides_all[~np.isinf(row_tides_all)]))[0][0]
    ...
    m1 = p1[:, mcol]
    m2 = p2[:, mcol]
    ##Assume we can get data  closest particles to binary...

    ai1 = np.ones((len(p1), 3)) * np.inf
    ai2 = np.ones((len(p1), 3)) * np.inf
    atot1 = np.ones((len(p1), 3)) * np.inf
    atot2 = np.ones((len(p1), 3)) * np.inf

    ##Getting tidal force from the star that is closest when the tidal force that is maximized.
    for jj in range(len(p1)):
        if ~np.isinf(p1[jj,0]) and ~np.isinf(p2[jj, 0]):
            closest_tag = closest[bin_key]
            tmp_close = path_lookup[closest_tag[tide_max_idx][0]][jj]
            tmp_close2 = path_lookup[closest_tag[tide_max_idx][1]][jj]
            pbin = np.vstack((p1[jj], p2[jj]))
            pclose = np.vstack((p1[jj], p2[jj], tmp_close, tmp_close2))
            tmp_a = pytreegrav.Accel(pbin[:, pxcol:pzcol + 1], pbin[:, mcol], pbin[:, hcol], theta=0.5, G=sfc.GN, method='bruteforce')
            tmp_b = pytreegrav.Accel(pclose[:, pxcol:pzcol + 1], pclose[:, mcol], pclose[:, hcol], theta=0.5, G=sfc.GN, method='bruteforce')
    ...
    tides_norm_series[bin_key] = tmp_acc_norm
# np.savez("tide_stars_close.npz".format(sys.argv[1]), tides_norm_series)
with open(base + aa  + "/tide_stars_close2.p", "wb") as ff:
